    ...
    Subshader
   {
       ...
       Pass
       {
           ...
           float4 BlurFrag(v2f input) : SV_Target
           {
               ...
               float isDark = step(upBound, input.uv.y) + step(input.uv.y, lowBound);
               float darkWeight = clamp(processing + 0.2, 0, 1);
               if (1 - isDark)
               {
                   input.uv = filpUV(input.uv);
                   float2 uv0 = input.uv + float2(-pixelWidth, pixelHeight) * 3;
                   float2 uv1 = input.uv + float2(-pixelWidth, -pixelHeight) * 3;
                   float2 uv2 = input.uv + float2(pixelWidth, pixelHeight) * 3;
                   float2 uv3 = input.uv + float2(pixelWidth, -pixelHeight) * 3;
